...
export default function CryptoSleuth() {
  ...
  const walletData: WalletCardInfo[] = result ? [
    { name: 'Ethereum', symbol: 'ETH', address: result.wallets.ethereum.address, balance: result.balances.ethBalance, icon: <EthIcon className="h-8 w-8" />, loading: isLoading },
    { name: 'Bitcoin', symbol: 'BTC', address: result.wallets.bitcoin.address, balance: result.balances.btcBalance, icon: <BtcIcon className="h-8 w-8" />, loading: isLoading },
    { name: 'Solana', symbol: 'SOL', address: result.wallets.solana.address, balance: result.balances.solBalance, icon: <SolIcon className="h-8 w-8" />, loading: isLoading },
    { name: 'BNB Smart Chain', symbol: 'BNB', address: result.wallets.bsc.address, balance: result.balances.bscBalance, icon: <BscIcon className="h-8 w-8" />, loading: isLoading },
    { name: 'Cardano', symbol: 'ADA', address: result.wallets.cardano.address, balance: result.balances.adaBalance, icon: <AdaIcon className="h-8 w-8" />, loading: isLoading },
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g., apple banana ..." {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <Button type="submit" className="w-full md:w-auto" disabled={isLoading}>
              ...
              {isLoading ? 'Searching...' : 'Generate and Check'}
            </Button>
          </form>
        </Form>
        ...
        {result && (
          <div className="space-y-6">
            <Card className="bg-primary/5 border-primary/20">
              <CardHeader>
                <CardTitle className="font-headline text-lg">Generated Seed Phrase</CardTitle>
              </CardHeader>
              <CardContent>
